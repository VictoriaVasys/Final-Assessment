...
function postLink() {
  var title = $('input[id="link_title"]').val();
  var url = $('input[id="link_url"]').val();
...
function makeFieldsEditable(e) {
  e.preventDefault();
  $(e.target).hide();
  var submitButton = $('<input type="button" class="update-link-button btn btn-defalt" value="Update Link"/>');
...
function updateAttributes(e) {
  e.preventDefault();
  var linkId = e.target.parentElement.dataset.linkId;
  var title = $(this).siblings('.title')[0].innerHTML.split(": ")[1];
  var url = $(this).siblings('.url')[0].innerHTML.split(": ")[1];
  ...
  var attrData = { title: title, url: url };
  $.ajax({
    type: "PATCH",
...
function markAsRead(e) {
  ...
  var url = e.target.previousElementSibling.previousElementSibling.previousElementSibling.previousElementSibling.textContent.split(': ')[0];
  ...
  $.ajax({
    ...
    url: "/api/v1/links/" + linkId,
    data: { read: true }
  }).then(addHotRead(url)).fail(displayFailure);
}
